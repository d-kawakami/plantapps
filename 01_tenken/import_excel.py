"""
Excel（日常点検表エクセル.xlsm）から点検項目をインポートするスクリプト
...
【使い方】
  ...
  パス省略時は同フォルダの "日常点検表エクセル.xlsm" を自動検索します。

  例:
    python import_excel.py
    python import_excel.py "日常点検表エクセル.xlsm"
    python import_excel.py "..\01_tenken\日常点検表エクセル.xlsm"
"""
...
def normalize_building(raw_name: str) -> str:
    ...
    建物名を正規化する
      - 週情報「（第N週）」を除去
      - prefix_normalize に登録された施設名で前方一致する場合は親名に統一
        （例: "調整槽A・B" → "調整槽"）
    """
